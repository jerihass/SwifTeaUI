...
| **Adaptive Panels** | Resizable layout primitives that respond to the size metrics above. | Implemented | `AdaptiveStack` swaps between expanded/collapsed layouts using terminal width; next explore per-panel size constraints and hooks into `TerminalMetrics` for more granular behaviors. |
...
| **Observable State Objects** | Property wrapper + observation hooks for class-based models (`@StateObject` analog). | Implemented | `ObservableObject` + `@StateObject`/`@ObservedObject` mirror SwiftUI’s ownership semantics; future work includes wiring publishers into selective render invalidation. |
| **ForEach Diffing** | Re-render only changed elements by leveraging user-provided IDs. | Implemented | Opt-in `.diffing(key:)` caches renders per ID, invalidating when the caller-provided key changes; see `docs/ForEachDiffing.md` for guidance on when to enable it. |
...
| **Layered Layout (ZStack)** | Explore overlapping/layered compositing akin to SwiftUI’s `ZStack`. | Implemented | `ZStack` overlays multiple `TUIView`s in order; future polish includes ANSI-aware blending modes and animation hooks. |
| **List Component** | Provide a high-level list abstraction similar to SwiftUI’s `List`. | Implemented | `List` now renders rows with customizable separators/spacing; future work includes section headers, selection handling, and scroll integration. |
| **Gradient Progress Meter (ANSI)** | Add stepped color gradients to progress bars for richer visuals in 8/16/256-color terminals. | Planned | Extend `ProgressMeter.Style` with a gradient palette option, color filled cells by position, keep empties neutral, and add snapshot tests for deterministic color sequencing. |
| **Gradient Progress Meter (Truecolor)** | Offer smooth RGB gradients on capable terminals while falling back gracefully elsewhere. | Planned | Add start/end (or stop) RGB style, emit `38;2` escapes per filled cell, gate on terminal capability or user opt-in, and mirror tests to assert truecolor escape output. |
| **Tab Bar Navigation** | SwiftUI-like tab strip for switching between primary scenes with number/arrow shortcuts. | Planned | Prototype top/bottom bar rendering, define keyboard shortcuts (1–9 plus arrows), preserve per-tab state to keep switches instant, and add a gallery demo to validate ergonomics. |
