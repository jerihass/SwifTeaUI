...
| **Async Action Effects** | Allow background tasks (network calls, timers, Task sleep) to emit reducer Actions without manual polling. | Proposed | 1) Design an `Effect` protocol that can spawn async work and an `ActionQueue` wired into the runtime loop; 2) Extend `TUIScene`/runtime with `dispatch(_:)` so effects enqueue follow-up actions that are drained before the next render; 3) Provide helpers (`Effect.run`, `Effect.timer`) plus documentation and a demo (TaskRunner) proving multiple concurrent async completions update state safely. |
| **Terminal Size Awareness** | Detect window resizing, enforce minimum sizes, and adapt rendering gracefully. | In Progress | Expand terminal metrics access beyond runtime, share minimum-size helpers with more demos, explore automatic layout compaction. |
