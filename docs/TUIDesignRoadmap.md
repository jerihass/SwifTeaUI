...
| **Table Layout Component** | Provide a declarative table/grid view with column definitions, headers, and alignment options. | Implemented | `Table` + `TableColumn` now cover measurement, dividers, and row styling; next polish is diff-aware updates and richer docs/screenshots (see `SwifTeaPackageListExample`). |
