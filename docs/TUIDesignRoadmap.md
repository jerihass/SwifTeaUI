...
| **Layered Layout (ZStack)** | Explore overlapping/layered compositing akin to SwiftUI’s `ZStack`. | Planned | Design a ZStack-style container that composes multiple `TUIView`s with z-order, define blending rules for ANSI escapes, and evaluate perf implications for repeatedly stacking overlays/backgrounds. |
