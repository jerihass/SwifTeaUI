...
| **List Component** | Provide a high-level list abstraction similar to SwiftUI’s `List`. | Planned | Define a `List` view that manages row separators, optional section headers, scrolling integration, and automatic focus/selection handling on top of existing stacks. |
