...
- Ordinary `Text`, `TextSpan`, `TextField`, and `TextEditor` content is literal-safe by default: terminal control scalars render as visible replacement characters before layout, while source bindings remain unchanged. Use `Text(trustedANSI:)` only for deliberately assembled, framework-owned presentation strings. See `docs/LiteralText.md` for the trust-boundary contract.
